...
export default function UpdateCustomer({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params);
    ...
    return (
        <>
            <div className="text-[18px] text-black/80 font-semibold mt-2">Update Customer {id}</div>
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-wrap space-y-4.5">
                    ...
                    <FormField
                        ...
                        name="locatName"
                        render={({ field }) => (
                            <DropdownSearch field={{ ...field }} data={languages} label="Local Name" />
                        )}
                    />
                    <FormField
                        control={form.control}
